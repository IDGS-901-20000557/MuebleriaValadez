...
export class AuthComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.loginForm.valid) {
      ...
      this.authService
        .login(email, crypto.SHA512(password).toString())
        .then((response: any) => {
          ...
          sessionStorage.setItem(
            ...
            response[0].nombres +
              ' ' +
              response[0].apellidoPaterno +
              ' ' +
              response[0].apellidoMaterno
          );
          sessionStorage.setItem('idPersona', response[0].idPersona);
          sessionStorage.setItem(
            'idDireccionSucursal',
            response[0].idDireccionSucursal
          );
          ...
          if (sessionStorage.getItem('idRol') === '1') {
            ...
            this.router.navigateByUrl('/repartir');
          }
        })
        .catch((error: any) => {
          Swal.fire({
